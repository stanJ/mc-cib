    ...
    var obj = {
        ...
        settings : {
            ...
            "phrases"           : {
            	...
            	'Ok': '确定',
            	'No matches': '无匹配数据',
            },         // empty object for english phrases
                    ...
                    .on('keydown', function (event) {
                        ...
                        switch (event.keyCode) {
                            case 8: // backspace;
                                ...
                                break;
                            /*
                             * 注释 by jc 180205 下面一起注释可以tab时切换表格里的输入框
                             */
//                          case 9:
//                          case 13:
//                              event.preventDefault();
//                              break;
                        ...
                        setTimeout(function () {
                            switch (event.keyCode) {
                            	/*
                            	 * 注释 by jc 180205 配合上面的注释 暂不清楚副作用
                            	 */
//                              case 9:  // tab
//                                  var next_rec = recid;
...
//                      tmp.focus();
        ...
        resizeRecords: function () {
            ...
            if (this.show.selectColumn) fwidth += 47;
...
//                    break;
        ...
        getColumnCellHTML: function (i) {
            ...
            var html = '<td id="grid_'+ this.name + '_column_' + i +'" col="'+ i +'" class="w2ui-head '+ sortStyle + reorderCols + '" ' +
                        ...
                        '    <div class="w2ui-col-header '+ (sortStyle ? 'w2ui-col-sorted' : '') +' '+ (selected ? 'w2ui-col-selected' : '') +'" '+(col.caption ? 'title="'+$('<div>'+col.caption+'</div>').text()+'"' : '')+'>'+
                        '        <div class="'+ sortStyle +'"></div>'+
                                (!col.caption ? '&#160;' : col.caption) +
                        '    </div>'+
                        '</td>';
...
var w2alert = function (msg, title, callBack) {
    ...
    if ($('#w2ui-popup').length > 0 && w2popup.status != 'closing') {
        w2popup.message({
            ...
            buttons : '<button onclick="w2popup.message();" class="w2ui-popup-btn w2ui-btn button--primary">' + w2utils.lang('Ok') + '</button>',
            onOpen: function () {
        ...
        w2popup.open({
            ...
            title     : title,
            body      : '<div class="w2ui-centered w2ui-alert-msg" style="font-size: 13px;">' + msg + '</div>',
            buttons   : '<button onclick="w2popup.close();" class="w2ui-popup-btn w2ui-btn button--primary">' + w2utils.lang('Ok') + '</button>',
            onOpen: function (event) {
            ...
            if (['enum', 'file'].indexOf(this.type) != -1) {
                ...
                if (html !== '') {
                    ...
                    if(options.style.length > 0){
                        ...
                        style = 'padding-top: ' + $(this.el).css('padding-top') + ';'+
        ...
        updateOverlay: function (indexOnly) {
            ...
            if (['list', 'combo', 'enum'].indexOf(this.type) != -1) {
                ...
                if ($(input).is(':focus')) {
                    ...
                    if (options.url == null && options.items.length === 0) msgNoItems = w2utils.lang('暂无数据');
