...
seajs.use('common/url.js', function(){

	app.host = url("hostname") + ":" + url("port");
	...
	app.seaAlias = {
		'jquery':'myjs/common/jquery-1.9.0.js',
		'highcharts': 'myjs/common/highcharts.js',
		'highcharts-more': 'myjs/common/highcharts-more.js',
		'solidgauge': 'myjs/common/solid-gauge.js',
		'iscroll':'myjs/common/iscroll.js',
		'template':'myjs/common/template.js',//模板
		'util':'myjs/common/util.js',//工具类&页面上可公用方法 utilObj&commonObj(图表的配置，nav&searchBar的事件绑定)
		'underscore':'myjs/common/underscore-min.js',
		'url':'myjs/common/url.js',
		'flatpickr':'myjs/common/flatpickr.js',
		'w2ui':'myjs/common/w2ui-1.5.rc1.js',
		'select2':'myjs/common/select2.js',
		'zh-CN':'myjs/common/i18n/zh-CN.js',
		'slider':'myjs/common/range.slider.js',
		'formdata':'myjs/common/formdata.js',
		'md5':'myjs/common/md5.js',
		'vconsole': 'myjs/common/vconsole.min.js'
	};

	app.OS="web";//app.OS="app";
	app.debug=false;//app.debug=true;
	app.version = '0.0.1';
	if(app.debug) app.version=new Date().valueOf();
	app.pageName = url('filename');
	...
	seajs.config({
		...
		paths:app.seaPath,
		map: [
	    	[ /^(.*\/js\/.*\.(?:js))(?:.*)$/i, '$1?'+app.version ],[ /^(.*\/css\/.*\.(?:css))(?:.*)$/i, '$1?'+app.version ]
	    ]
	});

	app.needChart = {
		"pandect": true,
		"cumulative-curve": true,
		"index": true,
		"no_loan_overview": true,
		"pipe-analysis": true,
		"intention": true,
	};

	var loadcss=["mycss/w2ui-1.5.rc1.css","mycss/select2.css","mycss/global.css","mycss/main.css","mycss/left-menu.css","mycss/"+app.pageName+".css"];
	if(app.pageName.indexOf("backend") > -1){
		loadcss.push('mycss/backend-main.css');
    }
	seajs.use(loadcss,function(){
		seajs.use(['jquery','iscroll',"template","underscore", "formdata","md5", "vconsole"],function(){
			seajs.use(["util","w2ui","select2"], function(){
			  if (utilObj.vconsole) {
				...
				var reIE = new RegExp("MSIE (\\d+\\.\\d+);");  
	           	...
	           	var fIEVersion = parseFloat(RegExp["$1"]); //获取IE版本

				if(trim_Version.indexOf("MSIE") != -1 && fIEVersion <= 8){
					//低版本IE
					$("body").append([
						"<div class='ie-wrap'>",
						"	<p>当前浏览器版本过低，推荐使用以下浏览器</p>",
						"	<ul>",
						"		<li><a class='firefox' href='http://www.mozilla.org/en-US/firefox/new/' target='_blank'></a></li>",
						"		<li><a class='anquan' href='http://se.360.cn/' target='_blank'></a></li>",						
					...
					//IE11或非IE
					//是否登陆
					if(app.pageName != "login" && !utilObj.userLogined){
						utilObj.gotoPageUri("login.html");
						return;
					}

					//判断权限
					if(app.pageName.indexOf("backend") > -1 && utilObj.userLogined.roleId != 2){
				        location.href = "login.html";
				    }
					
					var loadjs = ["myjs/left-menu.js","myjs/event.js","zh-CN"];

					if(app.needChart[app.pageName]){
						loadjs.push("highcharts");
					}

					if(app.pageName == "backend-layer"){
						loadjs.push("slider");
					}

					seajs.use(loadjs,function(LeftMenu){

						//左边菜单
						if(app.pageName != "login"){
							LeftMenu.getMenuList();
						}

						//w2ui日期控件本地化
						w2utils.settings.shortmonths = ["01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12"];
		                w2utils.settings.fullmonths = ["1月", "2月", "3月", "4月", "5月", "6月", "7月", "8月", "9月", "10月", "11月", "12月"];
		                w2utils.settings.shortdays = ["一", "二", "三", "四", "五", "六", "日"];
		                w2utils.settings.fulldays = ["周一", "周二", "周三", "周四", "周五", "周六", "周日"];

		                if(app.needChart[app.pageName]){
		                	seajs.use(["highcharts-more"]);
		                }

		                seajs.use(["myjs/"+app.pageName+".js"], function(){
		                	utilObj.detectSys(navigator.userAgent, navigator.platform);//侦测网页类型
							utilObj.forIOSCantUseMetaScale();//由于ios10 设置meta禁止页面缩放失效 阻止默认事件
							//iscroll配置
							$.os.ios?app.iScrollParam={
								momentum: true,
								bounce:true,
								disableMouse:true,
								deceleration:0.001,
